...
| Step | status    | 内容                                      | 完了条件                                                                 |
| ---- | --------- | ----------------------------------------- | ------------------------------------------------------------------------ |
| 1    | `planned` | template / asset / run の data model 設計 | migration、runtime guard、import/export schema、保存先が確定している     |
| 2    | `planned` | template 管理 UI                          | project admin が template を作成・編集・export/import できる             |
| 3    | `planned` | カスタム report 生成                      | 標準 report と custom report を選択生成でき、判定結果が JSON に残る      |
