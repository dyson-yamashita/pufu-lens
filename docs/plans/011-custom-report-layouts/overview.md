...
| 6    | `completed` | セキュリティ・公開境界の regression       | import、asset、PDF、public report で private data 混入を防ぐテストがある                                  |

**Outcome:** Step 5 では private/public の PDF endpoint と report detail の download link を追加した。PDF は Next.js route 上で report JSON からサーバー生成する最小 PDF とし、private は session + project member 認可、public は public project + published report 判定、DB 業務時間外 contract を既存 report API と揃えた。Step 6 では import asset manifest の path traversal / storage URI 形式 display name 拒否、custom layout PDF 生成時の private raw locator / storage URI / token / メールアドレス redaction、public report の未公開・private project 拒否を regression で確認した。
