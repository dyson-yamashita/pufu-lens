...
| `docs/plans/011-custom-report-layouts/overview.md`         | `active`    | 2026-06-30 | Issue #377 で追加。Issue #379 で Step 1 の template schema / data model 実装が完了。Step 2 の template 管理 UI と Step 3 のカスタム report 生成を実装（Issue 作成は手元環境で GitHub CLI (`gh`) が利用できず未実施）。カスタムレポート形式、汎用 `fixed_image`、判定パーツ、template import/export、PDF ダウンロードの実装計画。 |
