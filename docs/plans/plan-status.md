...
各39/52行・13件欠損を維持する。MENTIONS残差、qualityGate=false、実embedding/Chat/failure/remote未対応、
Step 6/7開始gate未達を保持する。詳細は[評価契約](../operations/backend-parity-evaluation.md)。
...
[評価契約](../operations/backend-parity-evaluation.md)。Step 6全完了 / Step 7実評価開始gate達成とは扱わず、
7B以降のrunner・実embedding・remote・performance / costを残す。既存品質・restore・削除gateと負荷スキップを維持。
...
Vectorize adapter / D1 outbox / repairとfetch-only embeddingのWorkers境界をローカル検証。
revision・配信epoch・tombstone・retry/dead-letterを追加。実D1/workerdとfake Vectorize契約試験を区別する。
詳細は[ADR-009](../adr/ADR-009-vectorize-outbox-repair.md)。remote gate、6E/7、既存品質・運用残件は維持する。
