...
| Step | status    | 内容                                                          | 完了条件                                                                 |
| ---- | --------- | ------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 1    | `planned` | event object schema / writer / reader を追加する。             | event object の write / list / read / validation の unit test が通る。     |
| 2    | `planned` | collection / parse / graph に artifact 書き込みを統合する。    | fixture ingest 後に raw / parsed / graph artifact が作成される。          |
| 3    | `planned` | `ingest:reconcile-artifacts` を追加する。                     | 既存 DB + Storage から欠損 artifact を backfill し、orphan を報告できる。 |
| 4    | `planned` | `ingest:restore --dry-run` を追加する。                       | 復元予定件数、欠損 artifact、衝突を DB 更新なしで報告できる。            |
| 5    | `planned` | `ingest:restore --apply` を追加する。                         | DB 空想定から関係性 graph まで再作成できる。                             |
| 6    | `planned` | docs / 運用手順 / smoke test を整備する。                     | 復元手順、検証 SQL、Storage 確認、secret 漏れ確認が docs に残っている。 |
