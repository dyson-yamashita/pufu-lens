...
| 2    | `planned`   | public/private chat response の source / tool call 表示方針を整理する。      | public response が機密値を出さず、private と同じ実行結果を追跡できる。                         |
| 3    | `planned`   | `public-report-chat-agent` など旧 public 専用経路の扱いを整理する。          | 現行経路と互換・検証用経路の責務が docs / tests で明確になっている。                           |
