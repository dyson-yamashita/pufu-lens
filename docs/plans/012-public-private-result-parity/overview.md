...
| 4    | `completed` | report detail / PDF の parity regression を補強する。Issue #435。                   | private/public が同じ private report JSON を描画・PDF 化する regression がある。               |
...
- `getPrivateReport` と `getPublicReport` が同じ private report JSON を返すことを regression test で確認する。
- private/public の detail で同じ report JSON を使ったうえで、PDF 生成入力となる `safeReportPdfLines` が一致することを確認する。
- public report detail / PDF の公開可否判定は DB metadata の public gate に残し、renderer / PDF content 生成は private report JSON ベースのまま維持する。
