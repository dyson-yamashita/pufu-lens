...
この Step は Mastra `generate-report` workflow 本体ではなく、workflow に載せる前の report generation core、CLI、API、UI、storage / DB 保存を最小確認する。Mastra Workflow 登録、Mastra UI での workflow trace 確認、Chat / Report Agent 連携は [Step 13d](step-13d-mastra-agent-workflow.md) で扱う。

### 実装する機能

- `generate-report` core / CLI（Mastra workflow 登録は Step 13d）
...
- Mastra 実装の残タスク:
  - `apps/web/src/report.ts` の `runGenerateReport`、provider、repository、storage 境界を再利用し、`apps/mastra` に `generate-report` workflow と必要なら `report-agent` を登録する。
  - Workflow 入力 schema、step graph、trace、LLM usage、JSON schema retry を Mastra Studio / Playground と自動テストで確認する。
