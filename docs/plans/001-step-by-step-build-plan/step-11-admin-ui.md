...
- レビュー対応: Server Action の mutation は `PUFU_LENS_ENABLE_ADMIN_ACTIONS=true` と `PUFU_LENS_ADMIN_USER_ID` を必須にし、`project_members.role='admin'` の対象 project member だけが実行できるようにする。未設定時は fail-closed とし、Auth.js session 連携は後続 API / auth 実装で request user に差し替える。
