...
Provider に渡す前回 report context は次の上限と共通 builder で bounded にする。文字数は Unicode code point 数で数え、全体は最大 16,000 文字かつ選択した provider の tokenizer で最大 6,000 token とする。Gemini provider は `countTokens` を使い、count API が利用不能な場合と extractive provider は UTF-8 byte 数を token 数とみなす安全側の fallback を使う。
