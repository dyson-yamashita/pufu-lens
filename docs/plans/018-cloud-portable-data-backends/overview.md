...
> 最新状態（2026-09-17、Issue #746）: PR #743 / #745によるrelational-only read/writeと設定を本番反映済み。
> build `8040c298-2491-4533-ac1a-977d935e441e`は14:34 JSTにSUCCESS、全8 unitはrelational-only。
> ユーザーは利用がほぼないことから停止・drain省略を明示承認した。通常の停止gate達成とは扱わない。
...
- 2CのPRではDDL migration、production DB実行、deploy、dual-write / shadow read、production switchを行わなかった。
  PR merge後のIssue #718で承認済みのlive inventory / rebuild / compareを実施した結果、差分が残ったため、
  全graphを再生成可能とは断定しない。live compareの差分ゼロまたは明示decisionを2D開始gateとして残す。
...
- 7B: backend runners / remote evaluation workflow
  - 2026-09-26、Issue #818で共通37chunk/36文書・3質問の認証付き実binding入口とoffline artifact bundle準備を追加。
    実workerd/D1＋明示fakeで検証。remote、全52ケースmapping、品質各39/52行・13件欠損、既存gate未達を維持。
    [本番移行までのcritical pathと承認対象](../../operations/cloudflare-parity-binding.md)を分離記録する。
  ...
  - 2026-09-26、Issue #800で最新mainから独立タスクとしてGraph read/mutationのローカルmappingを追加。
    ...
    2-hop契約の不一致を実測として保持する。fixture/hash/閾値を変更せず、残り13件・実embedding・remote、
    Step 6全完了 / Step 7開始gate・7C未実施を維持する。
...
| GCP keyword provider         | Step 3D品質補修・切替準備中 | LIKE OR pg_trgm word similarity / GiST候補をshadow比較とportable primary + PGroonga fallbackへ接続。数字境界の14-caseはIssue #763で合格。Issue #769で56-case portable品質合格、hybrid比較はbaseline未取得の2件で未達。Issue #772で実private Chat代表E2Eを両方式7/7確認。大規模負荷はユーザー指定でスキップ。全Chat網羅・本番切替は後続gate（ADR-005） |
