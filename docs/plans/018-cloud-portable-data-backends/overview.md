...
| GCP keyword provider         | Step 3D実装・切替準備中 | Step 3CのLIKE OR pg_trgm word similarity / GiST候補をshadow比較とportable primary + PGroonga fallbackへ接続準備。数字誤検出・広いholdout・大規模負荷・hybrid品質・本番切替は後続gate（ADR-005） |
