...
| GCP keyword provider         | Step 3D品質補修・切替準備中 | LIKE OR pg_trgm word similarity / GiST候補をshadow比較とportable primary + PGroonga fallbackへ接続。数字境界の14-caseはIssue #763で合格。Issue #769で56-case portable品質合格、hybrid比較はbaseline未取得の2件で未達。大規模負荷・全Chat E2E・本番切替は後続gate（ADR-005） |
