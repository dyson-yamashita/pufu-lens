...
- `(direction, local_actor_id, remote_actor_uri)` の follow identity、`follow_activity_uri` の generation、Activity URI receipt を組み合わせ、duplicate、Undo-before-Follow、旧 generation の Accept / Undo、再 follow を冪等化した。accepted / undone timestamp の整合は migration `0017_activitypub_follow_management` で `NOT VALID` 追加、`0019_activitypub_follow_constraint_validation` で `VALIDATE CONSTRAINT` を別 transaction 実行し、collection / project list の concurrent index を `0018_activitypub_follow_indexes` と DB contract test で固定した。
...
- Announceのcanonical objectはHTTPS限定のbounded fetchでdereferenceし、初回URLとredirect各hopに既存SSRF guard・domain blockを適用する。private / loopback / special-use / tunneling / NAT64、DNS rebinding、credential、fragment、過剰redirect、5秒timeout、1 MiB超過を拒否し、Create / Announceを共通mappingへ正規化する。
...
- HTML allowlist sanitizeをingestion・API・clientの境界で重ね、remote image / script / unsafe URLを除去した。project member認可済みの一覧APIとレポート画面は「自分のレポート」「外部レポート」を分離し、title、source Actor、domain、published time、sanitized summary、明示操作のoriginal URLだけを表示する。外部リンクは新規タブで `noopener noreferrer` を付与し、loading / empty / error / blocked状態を固定した。
