...
- retry exhaustedの安全なmetadata inspect、row lock / lease / status / `updated_at`再検証、message ID二重確認、固定形式change ref、queue更新と監査INSERTの同一transactionを持つrequeue / discard CLIを追加した。監査schemaはmigration `0024_activitypub_operations`とfresh `init.sql`へ同期し、payload / dedupe key / signature / response bodyをinspect / auditへ出さない。
