...
- Public: `/api/public/projects` または同等の server-side data loader で public project 一覧を返す。
- Public report / public chat: 既存の `/api/public/reports/[reportId]` と `/api/public/reports/[reportId]/chat` を維持する。
...
| Step 2 | `planned` | Auth.js session、user upsert、session user id 解決、必要な DB migration を実装する             | migration、ログイン / ログアウト、`users` upsert、session user id 取得の unit / route test が通る     |
...
- `/api/public/projects` を追加する場合の response schema、pagination、cache、rate limit、404 / empty state を定義する。
...
- provider callback で `users.email` / `users.name` を upsert し、session に `users.id` を載せる。
