...
- 新規ログインユーザーの global role は `user` などの制限ロールを既定にし、`users.role` の既定値が `admin` にならないよう migration で変更する。project 作成や system bootstrap の権限は global role と `project_members.role` の境界を明確にして判定する。
