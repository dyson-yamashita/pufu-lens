...
- **システムレポート:** `Level 2` のカードサーフェスを使って構造化し、ヘッダーには等幅フォント（JetBrains Mono）を使用します。データポイントを区切るため、区切り線を多めに使用します。標準レポートのセクション本文は private / public とも Markdown として表示し、見出し、箇条書き、強調、インラインコード、公開 HTTP(S) リンクを読みやすく描画します。
