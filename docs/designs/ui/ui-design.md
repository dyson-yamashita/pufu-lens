...
PDF レポートは Web のレポートと同じ情報階層を保ち、タイトル、対象期間・生成日時・Report ID、エグゼクティブサマリー、プ譜、各レポートセクションの順に構成します。プ譜は Web 画面で描画済みの `@goto-lab/pufu-editor` の ProjectScore を、エディター標準と同じ `dom-to-image` の PNG 出力で画像化して PDF に埋め込みます。本文には日本語埋め込みフォントを使用し、見出し・補助ラベル・本文のサイズ、余白、背景面、アクセントカラーによって読みやすい階層を付けます。複数ページではレポート名とページ番号をフッターに表示します。公開 PDF でも既存の機密情報マスキングを維持します。
...
- **Graph Query コントロール:** `Preset:` 選択の下に `Document count:` 上限（50 / 100 / 200 / 500、既定 50）と `Layout:` を 2 列で配置します。その下（専用 spacing 付き）に optional な `Start:` / `End:` の HTML date input を 2 列で配置します。各入力ラベルはコロン付きで表示します。両方 blank が既定で、blank 側は unbounded です。loading 中は Document count / Start / End を disable します。Graph Query ステータス badge は normalized graph 内の Document ノード数（`documentCount`、1 件のとき `1 document`、それ以外 `N documents`）を表示し、Raw Result パネルは AGE 生行数を表示します。Graph のコンテナサイズ変更時は viewport の resize / fit だけを行い、最大化・最大化解除で選択中の graph layout や node の model position を再計算しません。
...
- **Chat 待機表示:** assistant 応答生成中は、視認性を高めた三点リーダーを伴う `Thinking...` を表示します。その直下に、server から workflow stream された現在 stage の日本語ラベルを muted gray（`var(--muted)`）の 12px テキストで表示します。private / public chat とも同じ workflow ストリーミング方式を使い、LLM 検索計画では `質問の見方を整理しています`、`検索語を展開しています`、その後に検索・関連確認・時系列確認・回答生成の現在 stage を表示します。ラベルは `data-testid="chat-assistant-message-{index}-stage"` を持ち、`aria-live="polite"` で更新します。モバイルでも `Thinking...` と stage ラベルが重ならないよう縦積みにします。分類結果や実際の検索語は画面へ露出しません。
...
- **Hybrid search 上限:** Project Settings では、回答生成へ最終的に渡す document 数を 1〜20 の整数で設定します。既定値は 5 とし、private / public chat の共通 workflow に適用します。
...
- **システムレポート:** `Level 2` のカードサーフェスを使って構造化し、ヘッダーには等幅フォント（JetBrains Mono）を使用します。データポイントを区切るため、区切り線を多めに使用します。標準レポートのセクション本文は private / public とも Markdown として表示し、見出し、箇条書き、強調、インラインコード、公開 HTTP(S) リンクを読みやすく描画します。各セクションの source 一覧は件数を示す折りたたみに収め、private / public とも初期状態では閉じます。
