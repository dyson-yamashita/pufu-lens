...
- **Private Chat 待機表示:** assistant 応答生成中は `Thinking...` を表示します。その直下に、server から stream された現在 stage の日本語ラベルを muted gray（`var(--muted)`）の 12px テキストで表示します。ラベルは `data-testid="chat-assistant-message-{index}-stage"` を持ち、`aria-live="polite"` で更新します。モバイルでも `Thinking...` と stage ラベルが重ならないよう縦積みにします。public chat は従来どおり generic `Thinking...` のみとし、private workflow metadata を表示しません。
