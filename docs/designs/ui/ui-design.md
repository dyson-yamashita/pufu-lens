...
- **Private Chat 待機表示:** assistant 応答生成中は `Thinking...` を表示します。その直下に、server から stream された現在 stage の日本語ラベルを muted gray（`var(--muted)`）の 12px テキストで表示します。LLM 検索計画では `質問の見方を整理しています`、`検索語を展開しています`、その後に検索・関連確認・時系列確認・回答生成の現在 stage を表示します。ラベルは `data-testid="chat-assistant-message-{index}-stage"` を持ち、`aria-live="polite"` で更新します。モバイルでも `Thinking...` と stage ラベルが重ならないよう縦積みにします。分類結果や実際の検索語は画面へ露出せず、public chat は従来どおり generic `Thinking...` のみとします。
