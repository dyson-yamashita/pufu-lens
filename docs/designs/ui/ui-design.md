...
PDF レポートは Web のレポートと同じ情報階層を保ち、タイトル、対象期間・生成日時・Report ID、エグゼクティブサマリー、プ譜、各レポートセクションの順に構成します。プ譜は既存のレポート用 ProjectScore モデルから PDF 上へベクター描画し、ブラウザのスクリーンショットには依存しません。本文には日本語埋め込みフォントを使用し、見出し・補助ラベル・本文のサイズ、余白、背景面、アクセントカラーによって読みやすい階層を付けます。複数ページではレポート名とページ番号をフッターに表示します。公開 PDF でも既存の機密情報マスキングを維持します。
