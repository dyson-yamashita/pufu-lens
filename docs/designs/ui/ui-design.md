...
- **Graph Query コントロール:** Preset 選択の下に Documents 上限（50 / 100 / 200 / 500）と Layout を 2 列で配置します。その下（専用 spacing 付き）に optional な Start / End の HTML date input を 2 列で配置します。両方 blank が既定で、blank 側は unbounded です。loading 中は Documents / Start / End を disable します。Graph Query ステータス badge は normalized graph 内の Document ノード数（`documentCount`、1 件のとき `1 document`、それ以外 `N documents`）を表示し、Raw Result パネルは AGE 生行数を表示します。
