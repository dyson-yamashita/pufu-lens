...
レポート一覧はテーブルを基本にし、期間、生成日時、要約、生成種別を比較しやすくする。生成種別は `manual` を「手動」、`scheduled` を「定期（周期）」、`scheduled_backfill` を「定期 backfill（周期）」として表示する。生成ボタンはヘッダー右側に置き、生成中は進行中バッジと無効化状態を表示する。

project member のレポート一覧の下には定期レポート設定パネルを置く。project admin は `なし` / `週次` / `月次` / `年次` を保存でき、一般 member には現在周期を読み取り専用で表示する。非 member の公開レポート一覧には private な schedule 設定・実行状態を表示しない。パネルでは `Asia/Tokyo` の実行時刻、次回実行、前回成功・失敗、retry 回数と安全なエラー要約、period run の status 集計、残り backfill 件数、直近 period を確認できるようにする。保存中はボタンに進行状態を表示する。
