...
`/projects/[projectSlug]` は、プロジェクト基本情報に続いて最新定期 report 由来の現在状況を表示する。対象期間と生成日時、短い状況説明、プ譜、アセット、課題の順に配置する。アセットと課題は desktop で2カラム、狭い viewport で1カラムにする。定期 report がない、または最新 report が旧形式で `project_overview` を持たない場合は、次回生成後に表示されることを示す空状態にする。読込失敗時も基本情報と Reports / Chat 導線は維持する。

public project では、元 report が未公開でもマスク済み Overview を匿名表示する。元 report へのリンクは member には private URL、匿名利用者には report 自体が公開済みの場合だけ public URL を表示する。プ譜の client payload には document ID、canonical URI、storage URI、元の未公開 report ID を含めない。private project は従来どおり member だけが閲覧できる。
...
├────────────────────────────────────────────────────────────┤
│ Report Table                                                │
│ Title | Generation | Period | Schema | Created              │
├────────────────────────────────────────────────────────────┤
│ 定期レポート設定                                          │
│ 周期 / 次回実行 / 前回成功・失敗 / retry / period 状態    │
└────────────────────────────────────────────────────────────┘
```

レポート一覧はテーブルを基本にし、期間、生成日時、要約、生成種別を比較しやすくする。生成種別は `manual` を「手動」、`scheduled` / `scheduled_backfill` をどちらも「定期（周期）」として表示し、内部処理名の `backfill` は一覧へ露出しない。生成ボタンはヘッダー右側に置き、生成中は進行中バッジと無効化状態を表示する。

project member のレポート一覧の下には定期レポート設定パネルを置く。project admin は `なし` / `週次` / `月次` / `年次` を保存でき、一般 member には現在周期を読み取り専用で表示する。非 member の公開レポート一覧には private な schedule 設定・実行状態を表示しない。パネルでは `Asia/Tokyo` の実行時刻、次回実行、前回成功・失敗、retry 回数と安全なエラー要約、period run の status 集計、直近 period を確認できるようにする。残り backfill 件数は `pending` と重複するため別表示しない。保存中はボタンに進行状態を表示する。

初回有効化で同じ周期の定期 report が存在しない場合だけ、利用可能な最古のデータを含む period の開始日から現在進行中 period の前日までを1件の履歴 report として backfill queue へ登録する。週次を選んだ場合も過去週ごとに分割しない。初回履歴 report 後の通常実行は calendar period ごとの周次・月次・年次のままとする。同一周期の再保存と有効周期から別の有効周期への変更では即時 backfill を開始せず、次回 slot から新しい周期を使う。この違いを設定パネル内で常時説明する。

主要 `data-testid`:

- `report-schedule-panel`
- `report-schedule-frequency-input`
- `report-schedule-save-button`
- `report-schedule-member-readonly`
- `report-schedule-run-summary`
- `report-schedule-backfill-remaining`
...
Report Document は文書本文として読みやすい幅を保つ。本文カラムは最大 `880px` を目安にし、表やメトリクスがあるセクションだけ 12 カラム幅へ拡張できる。Report Context にはアウトライン、出典、関連レポート、共有操作を置く。`recurrence` がある定期 report では、summary と本文の間に周期、change summary、増加・新規、減少・解消、継続事項の共通差分パネルを表示し、空の分類は省略する。手動 report ではパネルを表示しない。`custom_layout` を持つレポートでは、標準の Sections 表示に代えて template snapshot の row / columns / title / fixed text / image placeholder / slider judgement / classification result / copyright を同じ Report Document 内に描画する。private と public の表示差分はアクセス制御と非公開 source 表示の扱いに限定し、custom layout の構造、判定結果、差分パネルは同じ private report JSON を使う。
...
差分パネルの主要 `data-testid` は private で `report-recurrence`、public で `public-report-recurrence` とし、frequency、summary、各 list は同じ prefix の suffix で識別する。
...
Actor 統合は特殊操作として通常は折りたたみ、展開後に active Actor の select から統合先と統合対象を選択する。project admin が merge を確定した場合は、代表 Actor に alias と relational 参照を寄せ、吸収元 Actor は物理削除せず `merged` status と `merged_into_actor_id` で無効化管理する。AGE graph が設定済みの場合、relational 更新と graph reconcile は同じ PostgreSQL transaction で実行し、graph の構文エラーや安全に reconcile できない node 状態では relational 更新も rollback してエラーを返す。graph 未設定、両 Actor が同じ graph node を参照する場合、または吸収元 graph node がすでに存在しない場合は graph reconcile を安全な no-op として relational merge を確定する。
