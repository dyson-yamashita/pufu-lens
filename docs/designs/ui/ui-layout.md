...
Actor 統合は特殊操作として通常は折りたたみ、展開後に active Actor の select から統合先と統合対象を選択する。project admin が merge を確定した場合は、代表 Actor に alias、relational 参照、graph 参照を寄せ、吸収元 Actor は物理削除せず `merged` status と `merged_into_actor_id` で無効化管理する。
