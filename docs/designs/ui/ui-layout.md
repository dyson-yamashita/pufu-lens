...
`Actors` タブでは表示名、status、strong alias 数、weak alias 数、source type、primary alias、graph node を一覧する。Actor 名から詳細画面へ遷移し、基本情報、alias、無効化状態、関連する merge / reject 判断履歴を確認できる。

`Merge Candidates` タブでは候補 Actor の比較、confidence、一致理由、根拠 source を表示する。weak alias だけでは自動統合しない。project admin が merge を確定した場合は、代表 Actor に alias と relational 参照を寄せ、吸収元 Actor は物理削除せず `merged` status と `merged_into_actor_id` で無効化管理する。reject した候補は判断履歴に保存し、同じペアを候補として再表示しない。
