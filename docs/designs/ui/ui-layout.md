...
project member の一覧上部には定期レポート設定パネルを置く。project admin は `なし` / `週次` / `月次` / `年次` を保存でき、一般 member には現在周期を読み取り専用で表示する。非 member の公開レポート一覧には private な schedule 設定・実行状態を表示しない。パネルでは `Asia/Tokyo` の実行時刻、次回実行、前回成功・失敗、retry 回数と安全なエラー要約、period run の status 集計、残り backfill 件数、直近 period を確認できるようにする。

初回有効化で同じ周期の定期 report が存在しない場合だけ、完了済み過去 period を backfill queue へ登録する。同一周期の再保存と有効周期から別の有効周期への変更では即時 backfill を開始せず、次回 slot から新しい周期を使う。この違いを設定パネル内で常時説明する。
