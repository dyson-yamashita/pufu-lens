...
- Fedify 関連 package は `2.3.4` へ完全固定し、Node.js `>=22` を使う。Web は Next.js 16 `proxy.ts` convention と `manuallyStartQueue: true` を使い、配送処理は PostgreSQL-backed one-shot process に分離する。Step 1 fixture は既定で無効であり、本番 federation 有効化は後続 Step の対象である。
