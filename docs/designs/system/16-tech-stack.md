...
- Embeddingは `PUFU_LENS_EMBEDDING_PROVIDER` / `PUFU_LENS_EMBEDDING_MODEL` / `PUFU_LENS_EMBEDDING_DIMENSIONS` をingestionとquery検索で共有する。実装済みproviderはGeminiとOpenAI、`deterministic` はテスト専用とし、`NODE_ENV=production` では共有runtimeで選択を拒否する。
