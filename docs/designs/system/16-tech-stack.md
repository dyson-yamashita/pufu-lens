...
- `gemini-embedding-001` は可変次元を返せるため、DB の `vector(1536)` に合わせて embedding 呼び出し時は `GEMINI_EMBEDDING_DIMENSIONS=1536` 相当の設定を必須にする。次元を変更する場合は `document_chunks.embedding` / `report_chunks.embedding` の migration と全 embedding の再生成を同じ step で扱う。
