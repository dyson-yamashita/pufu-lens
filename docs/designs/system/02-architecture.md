...
Plan 017 Step 2 では production 用の project / aggregate `@all` Actor、Actor 単位の暗号化鍵 repository、WebFinger / Actor / followers / following / outbox / Article dispatcher を追加した。production proxy は `ACTIVITYPUB_ENABLED=1` のときだけ PostgreSQL-backed federation を single-flight で初期化し、初期化失敗時は federation route を `503` で fail closed にする。公開 endpoint は public かつ federation-enabled な project だけを解決し、private / disabled / missing project とその Article は `404` に統一する。
