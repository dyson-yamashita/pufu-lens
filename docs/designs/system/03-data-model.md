...
- migration `0016_activitypub_actor_endpoints`、`0017_activitypub_follow_management`、`0018_activitypub_follow_indexes`、`0019_activitypub_follow_constraint_validation` と fresh `init.sql` は同じ Actor / follow / activity / queue / federated report 制約・index を持ち、`pnpm db:schema-drift` で同期を検証する。既存 DB の follow timestamp CHECK は `0017` で `NOT VALID` 追加、`0019` で `VALIDATE CONSTRAINT` を別 transaction 実行し、ADD transaction の lock を validation 中まで保持しない。follow index は `0018` で `DROP INDEX CONCURRENTLY` / `CREATE INDEX CONCURRENTLY` を個別実行し、中断後の invalid index も再試行時に再構築する。migration version の記録は全 statement 成功後に migration runner だけが行う。offline `pnpm db:migrate --check` は DB 接続なしで全 migration 本文の parse 契約も検証する。
