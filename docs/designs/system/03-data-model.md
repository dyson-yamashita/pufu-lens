...
- `report_schedule_period_runs` は同じ project・frequency・period を一意にし、report が生成されない `skipped` も理由と完了時刻を持つ履歴として残す。`succeeded` は report ID と完了時刻を必須とし、それ以外の状態では report ID を持たない。生成 report との相互参照は period run ID と project ID の複合制約で越境・不整合を拒否する。
