...
- migration `0016`〜`0019` に加え、`0020_activitypub_report_publication_outbox` と `0021_activitypub_validate_step4_constraints` を fresh `init.sql` と同期し、`pnpm db:schema-drift` で検証する。Step 4 の attempt / follow timestamp CHECK と queue / activity の固定 `last_error_code` allowlist CHECK は `0020` で `NOT VALID` 追加し、`0021` で別 transaction の `VALIDATE CONSTRAINT` を行う。migration version の記録は全 statement 成功後に migration runner だけが行い、offline `pnpm db:migrate --check` は全 migration 本文の parse 契約も検証する。
