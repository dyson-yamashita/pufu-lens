...
- `reports.generation_kind = 'manual'` では schedule metadata を持たず、`scheduled` / `scheduled_backfill` では canonical frequency と `schedule_period_run_id` を必須にする。期間列挙、前回定期 report 解決、前回 private JSON の project・frequency・period 再検証、bounded な差分生成は実装済みである。差分の `frequency` と `previous_report_id` は DB metadata を正として private report JSON の optional `recurrence` に保存し、dispatcher と UI は定期レポート計画の後続 Step で実装する。
