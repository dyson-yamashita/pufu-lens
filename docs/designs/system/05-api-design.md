...
- Public Chat API は信頼プロキシが付与した `x-forwarded-for` の右端の有効値（なければ `x-real-ip`、最後に anonymous bucket）+ report id 単位で厳しめに rate limit する。クライアントが任意に付与できる左端値は rate limit key として信用しない。初期値は 1 時間 10 request、1 日 50 request、質問長 2000 文字を上限にする。
