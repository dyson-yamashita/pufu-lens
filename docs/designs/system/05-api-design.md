...
- Public Chat API は Next.js が解決した client IP 相当値 + report id 単位で厳しめに rate limit する。初期値は 1 時間 10 request、1 日 50 request、質問長 2000 文字を上限にする。ブラウザが直接送った `x-forwarded-for` / `x-real-ip` は rate limit key として信用せず、`request.ip` が得られない環境では anonymous bucket に集約する。
