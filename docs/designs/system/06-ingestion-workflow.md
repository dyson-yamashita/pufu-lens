...
管理 UI で data source を作成する場合は、同じ transaction で対象 data source 固有の built-in parser profile と default approved version を作成し、active version に設定する。これにより、新規 data source は追加直後から default parser で ingest できる。既存 data source や CLI 収集で不足している default parser は、parse 実行時の seed 処理で補完できる。
