...
4c-github-lifecycle. bounded reconciliation / backfill CLI で既知 item を logical source 順に安定巡回し、
                      open → closed / merged と closed → open の reopen を軽量確認する。
                      lifecycle-only 更新は `lifecycleOnly=true` として parse / chunk / graph の
                      metadata / Document properties 更新に限定する（詳細: docs/operations/github-lifecycle.md）
