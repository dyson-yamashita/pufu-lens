...
| ツール                 | 役割                                                                                                                                                                                                                                                                               |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hybrid-search`        | provider-neutral な semantic / keyword candidate capability から順位付き候補を取得し、Core RRF で関連チャンクを返す（document_id 付き）。現行 GCP profile は pgvector / PGroonga adapter を使う。Agent 入力は `{ query, limit }` のみで、query embedding は server-side で生成する |
