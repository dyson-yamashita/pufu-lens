...
| `graph-query`          | provider-neutral な Graph read capability に `projectId` と `seedDocumentIds` を渡し、SAME_AS / RELATED_TO 1-hop と shared Topic MENTIONS 2-hop で関連 document を補助的に広げる。現行 GCP profile は AGE adapter を使う                                                           |
