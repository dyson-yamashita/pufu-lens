...
- Issue #771の[実Chat E2E](../../operations/live-chat-e2e.md)では実Next認証・Mastra・Gemini・embedding・DB・表示・履歴を接続し、両方式7/7・障害時1/1成功。代表private Chat経路の機能確認であり、当初source比較1件と回答表現（Issue #772）が未達だった。Issue #772で内部診断のcontext分離・引用指示を補修し、両方式7/7で必要な本文リンク・事実を確認、内部診断混入0件。今回のsource overlapは全件1.0だが、旧v1と既存比較gateは維持する。Issue #775で匿名public Chat・公開artifact表示も両方式8/8で確認した。脚注記号が残る書式の限界、大規模負荷スキップ、非Web資料・非空Graph等の網羅と本番観測・restoreは残す。
