...
Public Chat でも `editing` metadata は返せるが、公開 report / public context bundle の範囲に制限する。内部調査や private risk scan に見える質問は、公開情報の要約・説明または拒否に寄せ、DB / raw / parsed / graph への探索を示唆しない。
