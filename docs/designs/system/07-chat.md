...
- tool call: `vector-search` / `graph-query` / `document-fetch` / `raw-document-fetch` / `parsed-doc-fetch` を同一 API 内で順に実行し、回答に `sources` と `toolCalls` を含める。`vector-search` は pgvector 上位候補と PGroonga の `document_chunks.content` キーワード候補を内部で最大 200 件まで集め、`0.75 * vector_score + 0.25 * keyword_score` の hybrid score で document ごとの最良 chunk を返す。private source には title / docType / canonicalUri に加え、`documents.summary` または `document_chunks.content` 由来の短い `snippet` を含め、Agent は snippet を回答根拠として使う
