...
各 section の `sourceLocator` は adapter 内部で raw / parsed を辿れるが、**レスポンスに private raw locator、内部 storage URI、parsed URI を含めない**。

##### trace / log / API response

- Mastra trace、server log、Private Chat API response には **`traceSummary`、section count、`limits.truncated` のみ** を残す。
