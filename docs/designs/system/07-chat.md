...
| ツール                 | 役割                                                                                                                                                                                                                                                                     |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `vector-search`        | `document_chunks` から同じ embedding provider / model の pgvector 意味的類似度候補と PGroonga の本文キーワード候補を取得し、RRF で関連チャンクを返す（document_id 付き）。Agent 入力は `{ query, limit }` のみで、query embedding は server-side で生成する              |
...
期間は `startAt` inclusive / `endAt` exclusive の strict ISO-8601 instant とし、最大幅は 3660 日とする。期間表現と「取り組み」「活動」「実績」などの集約用語を除いた topic が空なら、本文に年や集約用語が明記されていることを要求せず、project scope と `documents.occurred_at` の期間だけで候補を取得する。topic が残る場合は既存の title / summary / chunk 検索も併用し、「プ譜友の会」のような内部助詞を含む固有名詞は保持する。期間表現を認識した場合は LLM の分類結果に依存せず timeline branch を実行し、候補数は chat response の source 上限と同じ最大 10 件に制限する。
...
- tool call: `vector-search` / `graph-query` / `document-fetch` / `raw-document-fetch` / `parsed-doc-fetch` を基本順として使い、時系列・経緯・履歴・流れ・期間集約を問う質問では `timeline-search` を追加で実行する。回答には `sources` と `toolCalls` を含める。`vector-search` の query embedding は document chunk 保存時と同じ `PUFU_LENS_EMBEDDING_PROVIDER` / `PUFU_LENS_EMBEDDING_MODEL` / 1536 次元で生成し、pgvector 候補は同じ `embedding_model` の chunk に限定する。query が空の場合は pgvector のみを使い、query がある場合は pgvector 上位候補と PGroonga の `document_chunks.content` キーワード候補（`&@~ pgroonga_query_escape(...)`、`pgroonga_score` 降順）を内部で最大 200 件まで集める。異なる score 尺度を直接加算せず、各順位を `1 / (60 + rank)` とする Reciprocal Rank Fusion（RRF）で統合し、document ごとの最良 chunk を返す。primary vector は最大 15 件を取得後、`gemini-embedding-2` では cosine distance 0.6 以下・相対距離 window・分布上の最大の崖で決定論的に選別し、その他のmodelでは絶対距離閾値を無効化して相対距離windowと分布上の崖を使う。選別後は同じ raw document、正規化 canonical URI、意味のある正規化 title の重複を決定論的に除外し、`comparison` / `relation` では document type が採用枠の 2/3 を超えないようにする。model変更時は `pnpm chat:measure-distances -- --project <slug> --query <query>` で再計測して閾値を設定する。score 付き vector 候補だけから（多様性 / docTypeQuota 適用前の件数で）`strong` / `weak` / `none` の retrieval confidence を判定し、`retrievalContext` の instruction にのみ含める。score と confidence は chat response、履歴、UI、public response には出さない。`timeline-search` は通常の時系列質問では `documents.occurred_at ASC NULLS LAST, documents.updated_at ASC` の候補を返し、期間指定時は `startAt <= occurred_at < endAt` を追加する。private source には title / docType / canonicalUri に加え、`documents.summary` または `document_chunks.content` 由来の短い `snippet` を含め、Agent は snippet を回答根拠として使う
