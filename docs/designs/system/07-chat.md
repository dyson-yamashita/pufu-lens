...
| ツール                 | 役割                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hybrid-search`        | provider-neutral な semantic / keyword candidate capability から順位付き候補を取得し、Core RRF（`k=60`）で関連チャンクを返す（document_id 付き）。現行GCPの既定はpgvector / PGroonga adapterで、Step 3Dは明示的なshadowまたはportable primary + PGroonga fallbackの準備だけを追加する。Agent入力は `{ query, limit }` のみで、query embeddingはserver-sideで生成する |
| `graph-query`          | provider-neutralなGraph read capabilityに`projectId`と`seedDocumentIds`を渡し、SAME_AS / RELATED_TO 1-hopとshared Topic MENTIONS 2-hopで関連documentを補助的に広げる。Step 2D compositionはAGE primary・既定offで、combined mode時だけ固定10%をrelational shadow比較する                                                                                             |
...
番号指定の有無によらず、detail取得で得た資料のsummary（summaryがない場合は先頭chunkの抜粋）を、回答生成用の `retrievalContext.sources[].documentSummary` に最大700文字で併記する。hybridが選んだchunkのsnippet・ID・indexは保持し、summaryだけに含まれる対象機能や事実も回答生成へ渡す。最終選定されなかった資料のsummaryは含めず、公開response・履歴のsource形状は変更しない。summaryも他の根拠と同じ未信頼コンテンツとして扱う。追加検索は行わず、既存のdetail取得結果を利用する。
...
#### Graph coverage pass（Issue #648）

hybrid search の採用 document を seed として、Graph を **coverage auditor** として使う。単純な追加 source 取得ではなく、seed 周辺の関連 document 漏れを relation 別の bounded pool で広げ、元の質問 / query plan で chunk evidence を再確認したうえで hybrid だけでは拾えない関連 document を最終 context へ補完する。

| relation     | hop | 内部候補 pool 上限（初期値） |
| ------------ | --- | ---------------------------- |
| `SAME_AS`    | 1   | 2                            |
| `RELATED_TO` | 1   | 5                            |
| `MENTIONS`   | 2   | 5                            |
...
- Issue #771の[実Chat E2E](../../operations/live-chat-e2e.md)では実Next認証・Mastra・Gemini・embedding・DB・表示・履歴を接続し、両方式7/7・障害時1/1成功。代表private Chat経路の機能確認であり、当初source比較1件と回答表現（Issue #772）が未達だった。Issue #772で内部診断のcontext分離・引用指示を補修し、両方式7/7で必要な本文リンク・事実を確認、内部診断混入0件。今回のsource overlapは全件1.0だが、旧v1と既存比較gateは維持する。Issue #775で匿名public Chat・公開artifact表示も両方式8/8で確認した。Issue #777で非空Graph・raw原文Chatとparsed metadataの直接HTTP取得を両方式3/3確認した。parsed toolはDB metadata取得であり本文を読まない。LLMが同toolを選ばなかった2試行は証跡に保持する。脚注記号などの表現限界、大規模負荷スキップ、非Web資料等の網羅と本番観測・restoreは維持する。
...
5. **graph / timeline / detail:** vector 結果を seed に graph coverage pass を実行する。relation 別 bounded pool（SAME_AS=2 / RELATED_TO=5 / MENTIONS=5）で候補を取得し、元の質問 / query plan の hybrid search で chunk evidence を再確認する。evidence なし候補は除外し、`relation` / `comparison` / `cause` / `process` では Graph-only evidence を最終 document 上限内で最低 1 件予約できる。LLM の primary / secondary operation が `timeline` の場合、deterministic editing metadata が timeline の場合、または期間表現を認識した場合に timeline retrieval を実行する。期間指定時は正規化済み topic と期間境界を repository に渡し、期間なしの時系列質問は従来の質問文検索を維持する。選定候補に対して bounded detail retrieval を行う。detail step では hybrid 検索で採用した chunk の snippet / `chunkId` / `chunkIndex` を保持し、`document-fetch` から `occurredAt`、canonical URI、title、doc type、raw document id などの metadata だけを補完する。graph / timeline のみで hybrid 採用が無い document は従来どおり detail snippet を使う。GitHub source は質問分類に応じた lifecycle selection を最終 document 上限の適用前に実行する。`retrievalContext` は synthesis 内部向け `untrusted_external_content` として chunk provenance、GitHub lifecycle metadata、graph relation metadata（`relationType` / `seedDocumentId` / `hopCount`）を含め、private chat response、履歴、UI、public response には `chunkId` / `chunkIndex` / score 系 / lifecycle-only internal metadata / graph relation metadata / diagnostics を出さない。graph coverageの件数・除外理由はworkflow stateに保持し、回答生成用のretrievalContextには含めない。
6. **Agent 合成:** Workflow retrieval 結果は `requestContext.retrievalContext` / `workflowSources` / `workflowToolCalls` に加えて、内部用の query classification / query plan と元の質問を synthesis に引き渡す。GitHub資料が選定された場合だけlifecycle説明の指示を加え、open / closed / merged とstatus未確認を区別する。retrieval 本文は `untrusted_external_content` として囲み、本文内の命令、role 変更要求、tool 呼び出し要求には従わない。回答本文には実際に使った資料名を明記し、canonical URIがある場合はMarkdownリンクにする。検索内部の診断値は回答本文へ転記しない。Agent は tool による追加確認は可能だが、Workflow 初期 retrieval の有無を Agent だけに委ねない。classification / query plan / lifecycle metadata / graph diagnostics は public chat や最終 `ChatResponse` には公開しない。
7. **Next.js 実行経路:** `POST /api/projects/[projectSlug]/chat` と、public project / report の公開判定を通過した public chat API は、server-side で解決した `projectId` と server-side の `nowIso` を使って Mastra HTTP Workflow API（`create-run` → `/api/workflows/private-chat-search/stream?runId=...`）を呼び出す。既存 workflow schema の nullable `graphName` field は移行互換のため残すが Graph query には使用せず、Next.js は `null` を渡す。`nowIso` はブラウザrequestから受け取らず、Private / Public の共有Workflow clientが実行ごとに設定する。`Accept: application/x-ndjson` の場合は、Mastra workflow stream の `workflow-step-start` を NDJSON progress event に写像して browser へ proxy し、最後に `result` または generic `error` event を返す。public chat の `result` は公開 report に含まれる web source だけへ変換してから返す。JSON-only client も同じ registered Workflow を利用する。Mastra の失敗を記録するときは固定 reason と HTTP status のみを使い、上流 response body、質問本文、LLM 出力や例外メッセージをログへ出さない。
...
現行の public project/report chat は、Next.js が `projects.visibility = 'public'` と `reports.is_public = true` を確認した後、private chat と同じ `private-chat-search` Workflow に検証済み `projectId` を渡して実行する。nullable `graphName` field は移行互換として `null` を渡し、Graph read adapter が `projectId` から provider graph を解決する。Workflow の synthesis は private chat と同じ `project-chat-agent` を使う。`public-report-chat-agent`、`public-report-fetch`、`public-context-fetch` は redaction 済み public report JSON / public context bundle だけを扱う互換・直接回帰検証用の経路として残し、正規の public chat 実行経路とは混同しない。
