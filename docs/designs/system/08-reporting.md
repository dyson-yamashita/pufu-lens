...
enabledなproject ActorにserverまたはprojectのActivityPub追加promptが設定されている場合、Gemini providerは通常reportとは分離した`activitypub_summary`も生成する。server promptを先、project promptを後の文体・persona guidanceとして適用し、両方を未信頼データとしてJSON文字列化した後、schema、日本語、事実根拠、privacy / PII redaction、安全性を上書きできない最終guardrailを置く。`activitypub_summary`は非空かつ最大500 code pointとし、private report JSON、public report artifact、title、通常summary、sections、`project_overview`、`recurrence`へ転記しない。公開transactionでは`reports.activitypub_public_summary`だけへ渡し、promptなし、manual publish、extractive / legacy経路では通常の公開summaryをfallbackにする。server promptはaggregate `@all`のenabled状態に依存せず、project Actorがenabledな場合に適用する。
