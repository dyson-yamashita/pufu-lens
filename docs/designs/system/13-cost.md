...
GCE VM 上の PostgreSQL は常時稼働させる。DB 依存機能（チャット、管理 UI、取り込み状況、データソース管理、手動 ingestion、レポート閲覧）と定期 `curate-workflow` / `ingest-workflow` / `generate-report` / `source-sync-dispatcher` job は時刻による利用制限や VM 起動制御を前提にしない。
