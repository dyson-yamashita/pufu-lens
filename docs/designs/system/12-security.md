...
- Web runtimeからのFollow / Accept / Undo即時起動はADCのCloud access tokenを使い、対象ActivityPub dispatcher Jobの`run.jobs.run` / `run.jobs.runWithOverrides`だけを許可する。token取得とAPI呼出しをboundedにし、token、Activity / Actor / queue ID、payload、response body、例外本文をlogへ出さない。失敗時は固定error codeだけを記録し、PostgreSQL queueとScheduler fallbackを維持する。
