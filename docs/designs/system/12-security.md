...
bindingへアクセスしない。任意本文/SQL/URL入力を禁止する。Issue #794で限定remote検証・cleanupは承認済み。
...
- aggregate Actor profile / enable設定はglobal admin、project Actor profile設定はproject adminまたはglobal adminに限定し、project memberには追加promptを返さない。profile更新でpreferred username、Actor ID、canonical URL、署名鍵を変更せず、disabled Actorを暗黙に再有効化しない。
- iconは空、HTTPS absolute URL、またはcanonical origin内の`/`始まりpathだけを許可する。protocol-relative URL、HTTP、credential、fragment、control character、backslashによるauthority混同を拒否し、relative pathはActor応答生成時にもcanonical originと同じoriginへ解決されたことを再検証する。
- server / projectの投稿追加promptは最大2,000 code pointの未信頼な文体・personaデータとして扱い、logへ本文を出さない。provider入力では値を構造化してserver→project順に置き、末尾のschema、日本語、事実根拠、privacy / PII、安全性guardrailを上書きできない。生成した`activitypub_summary`は最大500 code pointで通常report artifactから分離する。
...
- Web runtimeからのFollow / Accept / Undo即時起動は、Mastra runtime SAと分離した専用App Hosting Web runtime SAのADC Cloud access tokenを使い、対象ActivityPub dispatcher Jobの`run.jobs.run` / `run.jobs.runWithOverrides`だけを許可する。Scheduler SAは対象Mastra serviceのinvokerだけ、Mastra runtime SAは3つのdispatcher Jobだけへresource-level IAMを持つ。token取得とAPI呼出しをboundedにし、token、Activity / Actor / queue ID、payload、response body、例外本文をlogへ出さない。失敗時は固定error codeだけを記録し、PostgreSQL queueとScheduler fallbackを維持する。
