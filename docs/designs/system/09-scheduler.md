...
GitHub / Drive / Gmail の data source は作成 transaction 内で毎日 06:00 `Asia/Tokyo` の schedule を作る。migration は新規 row の DB default だけを変更し、既存 schedule の `daily_time` / `next_run_at` は変更しない。既存 schedule を 06:00 へ移す場合は、project admin が Data Sources 詳細で対象 source ごとに明示変更する。project admin は同じ画面で ON / OFF と日次時刻を変更でき、変更時は次の wall-clock occurrence を UTC の `next_run_at` に再計算する。Web は自動 schedule を持たない。
