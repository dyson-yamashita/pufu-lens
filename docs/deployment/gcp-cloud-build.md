...
| Deploy Cloud Build SA  | Artifact Registry writer、Cloud Run / Jobs deploy、Cloud Run Jobs create / update / execute（migration job 含む）、Firebase App Hosting deploy、Service Account User、Service Usage Viewer、Browser または custom read role | 環境ごとに分離する。`_RUN_DB_MIGRATIONS=true` の deploy では `${_DB_MIGRATION_JOB}` の create / update / execute 権限が必要                                |
| Runtime SA             | Secret Manager accessor（`DATABASE_URL` など）、GCS access、VPC connector 利用                                                                                                                                              | Cloud Run service / jobs / DB migration job に attach する。migration job も `${_VPC_CONNECTOR}` 経由で PostgreSQL に到達する必要がある                    |
| App Hosting compute SA | Secret Manager accessor、GCS access、Cloud Run Invoker、App Hosting compute runner、Admin UI から Cloud Run Jobs API で workflow job を起動する場合は対象 Job の `run.jobs.run` / `run.jobs.runWithOverrides`               | backend に custom SA を使う場合は特に確認する。`WORKFLOW_INPUT_JSON` override を渡す ingest 起動では `roles/run.developer` または同等の custom role が必要 |
...
Firebase App Hosting で custom service account を使う場合は、App Hosting source bucket の read 権限、`roles/firebaseapphosting.computeRunner`、参照 secret への access grant を確認する。Admin UI の data source ingest は Web runtime から Cloud Run Jobs API の `jobs/{job}:run` を呼び、`WORKFLOW_INPUT_JSON` を container override として渡す。Web runtime SA が workflow job を起動する環境では、対象 Job に対して `roles/run.developer` を付与するか、`run.jobs.run` と `run.jobs.runWithOverrides` を含む custom role を付与する。
...
- Admin UI から data source ingest を実行し、Web runtime SA が対象 workflow job を起動できる（`run.jobs.run` / `run.jobs.runWithOverrides` 不足の 403 が出ていない）。
...
`cloudbuild.deploy.yaml` は、substitution 検証後に Mastra Server image build と Workflow Job image build を並列で開始する。Workflow Job image push 後に `run-db-migration` step が Cloud Run Job migration を `--wait` 付きで完了させ、Mastra Server と Workflow Jobs deploy はその barrier 後に開始する。Firebase App Hosting deploy は backend 側の deploy 完了後に実行する。これにより、新しい frontend が対応する backend より先に公開される時間差を避け、runtime rollout 前に schema migration が完了する。smoke check は Cloud Run service、Cloud Run Jobs、Web deploy の完了を待つ。
