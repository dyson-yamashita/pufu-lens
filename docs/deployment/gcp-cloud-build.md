...
| Deploy Cloud Build SA  | Artifact Registry writer、Cloud Run / Jobs deploy、Cloud Run Jobs create / update / execute（migration job 含む）、Cloud Scheduler create / update、Firebase App Hosting deploy、Service Account User、Service Usage Viewer、Browser または custom read role | 環境ごとに分離する。DB migrationと5分dispatcher Schedulerの更新権限が必要                                                   |
| Runtime SA             | Secret Manager accessor（`DATABASE_URL` など）、GCS access、VPC connector 利用、dispatcher Cloud Run Jobの実行                                                                                                                                               | Cloud Run service / jobs / DB migration job にattachし、Mastra Serverからdispatcher Jobを起動できるよう対象Jobにscopeを絞る |
