...
- chat hybrid search smoke: Mastra Server と `document_chunks` の `GEMINI_EMBEDDING_MODEL` が一致し、固有名詞 / Issue 番号を含む質問で `vector-search` が vector / keyword 候補を RRF 統合して返すこと
- 実行後 smoke:
