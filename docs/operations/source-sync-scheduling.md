...
source ごとの Cloud Scheduler resource は作らない。Web source は自動 schedule を持たず、管理画面または既存の collect / ingest 導線から手動実行する。Synthetic Monitor は `POST /internal/monitoring/v1/observations` で project 配下の source schedule 状態を読み取り専用で確認でき、`last_error` 本文や worker token は返さない。詳細は [Synthetic Monitor 運用手順](synthetic-monitoring.md) を参照。
