export interface PortableKeywordTerm {
  ...
  /** Empty disables trigram approximation for numeric phrases and literal punctuation. */
 ...
 * keep the existing trigram path only. Regex syntax from input is always escaped.
 */
...
/**
 * Requires every whitespace-delimited term after DB normalization. Punctuation stays literal.
  ...
  for (let index = 0; index < words.length; index++) {
    ...
    if (/^[a-z]+$/.test(word) && next !== undefined && /^[0-9]+$/.test(next)) {
      ...
      terms.push({
        literal: '',
        approximate: '',
        pattern: `(^|[^a-z0-9_])${label}[[:space:]]+${next}([^0-9]|$)`,
      });
      index++;
    } else {
      const plain = /^[\p{L}\p{M}]+$/u.test(word);
      terms.push({
        literal: escapeLike(word),
