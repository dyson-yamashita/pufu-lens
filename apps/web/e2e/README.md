...
| Admin route rejection             | `admin-ui.spec.ts`  | public user             | admin data sources / removed parser profiles route / settings | desktop            | fixture DB            | 未ログインでは login に戻り、管理情報が描画されない                                  |
...
| Private report list to detail     | `report-ui.spec.ts` | member user             | private report list / detail                                  | desktop            | API mock              | 一覧で手動・定期と周期を識別し、詳細の source を初期折りたたみから開ける             |
| Private report mobile readability | `report-ui.spec.ts` | member user             | private report detail                                         | mobile (`@mobile`) | API mock              | 主要 section と定期 report の差分が mobile viewport で表示される                     |
...
| Public report redaction and delta | `report-ui.spec.ts` | public user             | public report                                                 | desktop            | API mock              | private と共通の redacted 本文・定期差分を表示し、private project は 404 になる      |
