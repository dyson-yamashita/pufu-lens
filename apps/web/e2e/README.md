...
| Admin route rejection             | `admin-ui.spec.ts`  | public user             | admin data sources / parser profiles / settings | desktop            | fixture DB            | 未ログインでは login に戻り、管理情報が描画されない                                  |
| Admin operation controls          | `admin-ui.spec.ts`  | admin user              | data sources / parser profiles / settings       | desktop            | real DB + credentials | 運用画面の主要 control が安定した `data-testid` で操作・確認できる                   |
