...
5. Deploy the Mastra Server to Cloud Run after its image is pushed and the migration step finishes. The server receives `PUFU_LENS_GRAPH_TRANSITION_MODE` from `_GRAPH_TRANSITION_MODE` and `PUFU_LENS_KEYWORD_TRANSITION_MODE` from `_KEYWORD_TRANSITION_MODE`.
6. Deploy `${_ENV}-curate-workflow`, `${_ENV}-ingest-workflow`, `${_ENV}-generate-report`, `${_ENV}-source-sync-dispatcher`, `${_ENV}-report-schedule-dispatcher`, and `${_ENV}-activitypub-dispatcher` as Cloud Run Jobs after the jobs image is pushed and the migration step finishes. Every Job receives the same graph and keyword transition modes; the ActivityPub Job receives its fixed canonical origin and Actor key secret reference separately.
...
Keyword transition is independently server-owned. `_KEYWORD_TRANSITION_MODE` accepts `pgroonga-primary`, `pgroonga-shadow`, or `portable-primary`; the safe default is `pgroonga-primary`, and production validation requires it to match the single runtime `PUFU_LENS_KEYWORD_TRANSITION_MODE` entry in `apps/web/apphosting.yaml`. Step 3D preparation does not run production migration, backfill, shadow, or primary switching.
...
- Read and write access to the Cloud Build default bucket object
  `gs://${PROJECT_ID}_cloudbuild/pufu-lens/deploy-state/${_ENV}/apphosting-last-success`
  when `_FIREBASE_DEPLOY=true`; used to skip App Hosting deploy when web paths are unchanged since the last successful deploy.
